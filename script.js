...
/* ---------------- animated menubar icon: same wireframe shape-morph
   loop as the preloader, but small and runs forever (not tied to the
   preloader's lifecycle) ---------------- */
(function initMenubarIcon(){
  try{
    if(!window.THREE) throw new Error('three.js unavailable');
    const canvas = document.getElementById('menubar-icon-canvas');
    if(!canvas) return;
    const size = 18;
    ...
    const GRADIENT_STOPS = [[155,126,240],[240,130,196],[44,224,184]]; /* --purple, --pink, --teal */
    function gradientColor(t){
      t = Math.min(1, Math.max(0, t));
    ...
    function makeShape(idx){
      const geometry = new THREE.EdgesGeometry(shapeDefs[idx]());
      const pos = geometry.attributes.position;
      const colors = new Float32Array(pos.count * 3);
      for(let i = 0; i < pos.count; i++){
        const t = (pos.getX(i) + RADIUS) / (RADIUS * 2);
    ...
    let ptr = 0;
    let current = makeShape(ptr);
    scene.add(current);
    let next = null;
    const STAGE_MS = 1500, SHIFT_MS = 650;
    let stageStart = performance.now();
    let shifting = false, shiftStart = 0;
    function easeInOutCubic(t){ return t < 0.5 ? 4*t*t*t : 1 - Math.pow(-2*t+2,3)/2; }
    ...
    function animate(){
      ...
      current.rotation.x += dt * 0.5;
      current.rotation.y += dt * 0.7;
      if(next){ next.rotation.x += dt * 0.5; next.rotation.y += dt * 0.7; }

      if(!reduceMotion && !shifting && now - stageStart > STAGE_MS){
        shifting = true; shiftStart = now;
        ptr = (ptr + 1) % shapeDefs.length;
        next = makeShape(ptr);
        next.scale.setScalar(0.001);
      ...
      if(shifting){
        const p = Math.min(1, (now - shiftStart) / SHIFT_MS);
        const e = easeInOutCubic(p);
        current.scale.setScalar(1 - e);
        current.rotation.z += dt * 1.8;
        next.scale.setScalar(Math.max(0.001, e));
        next.rotation.z -= dt * 1.8;
    ...
    const canvas = document.getElementById('menubar-icon-canvas');
    const fallback = document.getElementById('menubar-icon-fallback');
